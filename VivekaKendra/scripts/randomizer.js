...
//"../images2/
stdImg[0]="ninja1.gif";	//100%
stdImg[1]="ninjaTweeter.gif";	//80%	
stdImg[2]="haveFlowers.gif";		//60%
stdImg[3]="catlick.gif";	//40%
stdImg[4]="snore.gif";	//30%
...
function setupRandomData()
{
loadPreferences();
initDataOnly();
}

//relative path to congratulatory image changes based on the launch mathods ( legacy, templated or generated)
function getCongratsImgPath()
{
	if((typeof g_renderingType === "undefined") || g_renderingType ==="L" || g_renderingType ==="" )
	{
		return congPath="../images2/";
	}
	else
	{
		return g_dynSheetMoodImagePath;
	}
...
function dspCongratulations()
{
	...
	var cPath=getCongratsImgPath();
	//relative path to congratulatory image changes based on the launch mathods ( legacy, templated or generated)
	cPath += (congImg[conIdx] == "") ? stdImg[conIdx] : congImg[conIdx];
	imgNode.src = cPath;
...
function checkPossibleAns(pAns, yAns)
{
	...
	do
	{
		...
		if(idx != -1)
			...
			theAns = ans;
		//ignore space?
		if(!(typeof ignoreSpaces === "undefined") && ignoreSpaces)
		{
			yAns=yAns.replace(/\s/g, '');
			theAns=theAns.replace(/\s/g, '');
		}
		
		//ignore case?
		if(!(typeof ignoreCase === "undefined") && ignoreCase)
		{
			yAns=yAns.toUpperCase();
			theAns=theAns.toUpperCase();
		}		
		if (yAns == theAns) 
		{
			return true;
