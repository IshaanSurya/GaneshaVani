...
element=element.parentNode;}while(element&&element!==div_drag);tables[j]=tables_nodeList[i];tables[j].redips={};tables[j].redips.container=div_drag;tables[j].redips.nestedLevel=level;tables[j].redips.idx=j;td=tables[j].getElementsByTagName('td');for(k=0,rowspan=false;k<td.length;k++){if(td[k].rowSpan>1){rowspan=true;break;}}
tables[j].redips.rowspan=rowspan;j++;}
for(i=0,group_idx=sort_idx=1;i<tables.length;i++){if(tables[i].redips.nestedLevel===0){tables[i].redips.nestedGroup=group_idx;tables[i].redips.sort=sort_idx*100;nested_tables=tables[i].getElementsByTagName('table');for(j=0;j<nested_tables.length;j++){nested_tables[j].redips.nestedGroup=group_idx;nested_tables[j].redips.sort=sort_idx*100+nested_tables[j].redips.nestedLevel;}
...
REDIPS.drag.obj_old=obj_old=obj||this;REDIPS.drag.obj=obj=this;clone_class=obj.className.indexOf('clone')>-1?true:false;table_top(obj);if(div_drag!==obj.redips.container){div_drag=obj.redips.container;init_tables();}
...
table_mini.redips={};table_mini.redips.container=el.redips.container;table_mini.redips.source_row=row_obj;form_elements(row_obj,table_mini.rows[0]);copy_properties(row_obj,table_mini.rows[0]);document.getElementById('redips_clone').appendChild(table_mini);offset=box_offset(row_obj,'fixed');table_mini.style.position='fixed';table_mini.style.top=offset[0]+"px";table_mini.style.left=offset[3]+"px";table_mini.style.width=(offset[1]-offset[3])+"px";return table_mini;}};row_drop=function(r_table,r_row,table_mini){var tbl=tables[r_table],ts=tbl.rows[0].parentNode,animated=false,tr,rp,src,rowIndex,delete_srow;delete_srow=function(){if(!animated&&obj_old.redips.empty_row){row_opacity(obj_old,'empty',REDIPS.drag.row_empty_color);}
...
else if(!moved){REDIPS.drag.myhandler_notmoved();}
...
else if(cloned){REDIPS.drag.myhandler_row_cloned();}
...
moved=true;if(X>obj_margin[3]&&X<window_width-obj_margin[1]){obj.style.left=(X-obj_margin[3])+'px';}
...
c=tbl.rows[ri].cells[ci];return[ri,ci,c];};

save_content=function(tbl,fldValues)
{		
	var query='',tbl_start,tbl_end,tbl_rows,cells,tbl_cell,t,r,c,d;
	tables.sort(function(a,b){return a.redips.idx-b.redips.idx;});
