	...
	drag:function(e){
		...
		if (this.targetobj.className=="dragDupe"){				
			...
			{				
				...
				clonedNode.style.left=Math.abs(dPos.x)+30;
				clonedNode.style.top=Math.abs(dPos.y)+30;
...
function isInDropCells(tblObj)
{
	var slots = tblObj.getElementsByClassName("dropcell");
	if(slots !=null )
	{
		return slots;
	}
...
function checkMulValuesByName(form,snap) 
{
	var dgObjOrder = [];
	var score = 0;
	var qIx=0;
	...
	if(tblObj == null)
		return;
	var cellObjs = isInDropCells(tblObj);
	if(cellObjs == null)
		return;
	...
	for (var i=cellSeq; i<cellObjs.length; i++) 
	{	
		var toNext=false;
		...
		for(var k=dgSeq;k<dgObjOrder.length;k++)
		{
			...
			{	//answer from all the elements in the dropcell
				if(ansStr.length > 0)
				{	//alert(aSeq + ") " + ansStr);
			...
			//if no items in the target drop cell
			if(ansStr.length ==0 && k==(dgObjOrder.length-1))
			{//	alert(aSeq + " : " + ansStr);
				orderedAns[orderedAns.length] = "";
				aSeq++;
				dgObjOrder[k].ansSeq=aSeq;
			}
	...
	for ( var i=0; i<orderedAns.length; i++,qIx++) 
	{
		...
		do
		{
			...
			//	alert(orderedAns[i] +" A Match? " + theAns);
				score++;
				for(var k=0;k<dgObjOrder.length;k++)
				{
